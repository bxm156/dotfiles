...
# IMPORTANT: Use -b flag with absolute path - installer defaults to relative ".local/bin"
...
if [[ "$SAFE_MODE" == true ]]; then
    ...
    exec sh -c "$(curl -fsLS get.chezmoi.io/lb)" -- -b "$HOME/.local/bin" init --apply "$CHEZMOI_GITHUB_USER"
fi
...
install_with_gum() {
    ...
    if ! "$gum_bin" spin \
        ...
        --show-error \
        -- sh -c "$(curl -fsLS get.chezmoi.io/lb)" -- -b "$HOME/.local/bin"; then
